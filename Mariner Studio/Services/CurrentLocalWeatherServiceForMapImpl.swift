...
class CurrentLocalWeatherServiceForMapImpl: CurrentLocalWeatherServiceForMap {
    ...
    func getWeather(latitude: Double, longitude: Double) async throws -> OpenMeteoResponse {
        ...
        print("🌦️ CurrentLocalWeatherServiceForMapImpl: Fetching weather data from URL: \(url.absoluteString)")
        ...
        do {
            ...
            print("✅ CurrentLocalWeatherServiceForMapImpl: Successfully decoded weather data")
            return weatherData
        } catch {
            print("❌ CurrentLocalWeatherServiceForMapImpl: Failed to decode weather data: \(error)")
    ...
    func getHourlyForecast(year: Int, month: Int, day: Int, latitude: Double, longitude: Double) async throws -> OpenMeteoHourlyResponse {
        ...
        print("🌦️ CurrentLocalWeatherServiceForMapImpl: Fetching hourly forecast for \(dateString) from URL: \(url.absoluteString)")
        ...
        do {
            ...
            print("✅ CurrentLocalWeatherServiceForMapImpl: Successfully decoded hourly forecast data")
            return forecastData
        } catch {
            print("❌ CurrentLocalWeatherServiceForMapImpl: Failed to decode hourly forecast data: \(error)")
            throw NetworkError.decodingError(error)
        }
    }
    ...
    func getMarineForecast(year: Int, month: Int, day: Int, latitude: Double, longitude: Double) async throws -> OpenMeteoMarineResponse? {
        ...
        print("🌊 CurrentLocalWeatherServiceForMapImpl: Fetching marine forecast for \(dateString) from URL: \(url.absoluteString)")
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 404 {
                print("ℹ️ CurrentLocalWeatherServiceForMapImpl: Location is not a marine location (404)")
                return nil
            }
            ...
            print("✅ CurrentLocalWeatherServiceForMapImpl: Successfully decoded marine forecast data")
            return marineData
        } catch {
            // If timeoutError, return nil instead of throwing error
            if let urlError = error as? URLError, urlError.code == .timedOut {
                print("⏱️ CurrentLocalWeatherServiceForMapImpl: Marine data request timed out. Likely not a marine location.")
                return nil
            }
            
            print("❌ CurrentLocalWeatherServiceForMapImpl: Failed to fetch marine forecast data: \(error)")
            throw error
        }
    }
}
