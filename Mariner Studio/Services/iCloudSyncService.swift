...
protocol iCloudSyncService {
    ...
    func downloadPhotos(for navUnitId: String) async throws -> [CloudPhoto]
    func deletePhoto(recordID: String) async throws
    func deleteNavUnitPhoto(_ photo: NavUnitPhoto) async throws // NEW: Delete by NavUnitPhoto
    func syncAllLocalPhotos() async
    func syncPhotosForNavUnit(_ navUnitId: String) async
    func setPhotoService(_ photoService: PhotoDatabaseService)
    func setFileStorageService(_ fileStorageService: FileStorageService)
...
class iCloudSyncServiceImpl: ObservableObject, iCloudSyncService {
    ...
    func uploadPhoto(_ photo: NavUnitPhoto, imageData: Data) async throws -> String {
        ...
        do {
            ...
            if let photoService = self.photoService {
                do {
                    try await photoService.updateNavUnitPhotoCloudKitRecordID(
                        photoId: photo.id,
                        recordID: recordID
                    )
                    print("✅ iCloudSyncService: Updated local database with CloudKit record ID for photo \(photo.id)")
                } catch {
                    print("⚠️ iCloudSyncService: Failed to update local CloudKit record ID: \(error.localizedDescription)")
    ...
    // NEW: Delete photo by NavUnitPhoto (uses CloudKit record ID if available)
    func deleteNavUnitPhoto(_ photo: NavUnitPhoto) async throws {
        guard let recordID = photo.cloudKitRecordID else {
            print("⚠️ iCloudSyncService: Photo \(photo.id) has no CloudKit record ID, skipping CloudKit deletion")
            return
        }
        
        print("🗑️ iCloudSyncService: Deleting photo \(photo.id) from CloudKit with record ID: \(recordID)")
        
        do {
            try await deletePhoto(recordID: recordID)
            print("✅ iCloudSyncService: Successfully deleted photo \(photo.id) from CloudKit")
        } catch {
            print("❌ iCloudSyncService: Failed to delete photo \(photo.id) from CloudKit: \(error.localizedDescription)")
            throw error
        }
    }
