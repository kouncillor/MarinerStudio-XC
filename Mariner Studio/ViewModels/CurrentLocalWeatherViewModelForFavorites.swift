...
class CurrentLocalWeatherViewModelForFavorites: ObservableObject {
    ...
    func initialize(
        ...
        print("🚀 CurrentLocalWeatherViewModelForFavorites: Initialized with location (\(latitude), \(longitude))")
    }
        ...
        weatherTask = Task {
            ...
            if let geocodingService = geocodingService {
                do {
                    ...
                    print("⚠️ CurrentLocalWeatherViewModelForFavorites: Geocoding error: \(error), continuing with coordinates only")
                    await MainActor.run {
                        locationDisplay = "Location at \(String(format: "%.4f", latitude))°, \(String(format: "%.4f", longitude))°"
                    }
                }
            ...
            if let weatherService = weatherService {
                do {
                    print("🌤️ CurrentLocalWeatherViewModelForFavorites: Fetching weather for location (\(latitude), \(longitude))")
                    ...
                    print("✅ CurrentLocalWeatherViewModelForFavorites: Weather data received, processing...")
                    ...
                    if !Task.isCancelled {
                        print("❌ CurrentLocalWeatherViewModelForFavorites: Weather API error: \(error)")
                        await MainActor.run {
