...
class NavUnitDetailsViewModel: ObservableObject {
    ...
    func cleanupDuplicatePhotos() async {
        guard let unit = unit else { return }
        
        print("🧹 NavUnitDetailsViewModel: Starting duplicate cleanup for nav unit: \(unit.navUnitId)")
        
        do {
            let removedCount = try await photoService.removeDuplicatePhotosAsync(navUnitId: unit.navUnitId)
            
            if removedCount > 0 {
                print("✅ NavUnitDetailsViewModel: Removed \(removedCount) duplicate photos")
                
                // Reload photos to reflect changes
                await loadLocalPhotos()
                
                await MainActor.run {
                    // Clear any stale sync status for removed photos
                    let currentPhotoIds = Set(localPhotos.map { $0.id })
                    photoSyncStatuses = photoSyncStatuses.filter { currentPhotoIds.contains($0.key) }
                }
            } else {
                print("✅ NavUnitDetailsViewModel: No duplicate photos found")
            }
        } catch {
            print("❌ NavUnitDetailsViewModel: Error during duplicate cleanup: \(error.localizedDescription)")
            await MainActor.run {
                errorMessage = "Error cleaning up duplicate photos: \(error.localizedDescription)"
            }
        }
    }
    
    // MARK: - Auto Sync with Throttling and Duplicate Prevention
    private func performAutoSyncIfEnabled() async {
        ...
        print("🔄 NavUnitDetailsViewModel: Starting enhanced auto-sync for nav unit: \(unit.navUnitId)")
        
        // Step 1: Clean up any existing duplicates first
        await cleanupDuplicatePhotos()
        
        // Step 2: Perform iCloud sync
        await iCloudSyncService.syncPhotosForNavUnit(unit.navUnitId)
        
        // Step 3: Clean up any duplicates that might have been created during sync
        await cleanupDuplicatePhotos()
        
        // Step 4: Reload photos after sync and cleanup
        await loadLocalPhotos()
        ...
        print("✅ NavUnitDetailsViewModel: Enhanced auto-sync completed for nav unit: \(unit.navUnitId)")
    }
    ...
    // Enhanced manual sync with duplicate cleanup
    func manualSyncNavUnit() async {
        ...
        print("🔄 NavUnitDetailsViewModel: Starting enhanced manual sync for nav unit: \(unit.navUnitId)")
        ...
        do {
            ...
            print("✅ NavUnitDetailsViewModel: Enhanced manual sync completed successfully")
            
        } catch {
            print("❌ NavUnitDetailsViewModel: Enhanced manual sync failed: \(error.localizedDescription)")
            await MainActor.run {
