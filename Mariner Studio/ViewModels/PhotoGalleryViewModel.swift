...
class PhotoGalleryViewModel: ObservableObject {
    ...
    func confirmDelete() async {
        ...
        do {
            print("🗑️ PhotoGalleryViewModel: Starting deletion of photo \(photoToDelete.id)")
            print("🗑️ CloudKit record ID: \(photoToDelete.cloudKitRecordID ?? "none")")
            
            // Delete from CloudKit first (if it exists there and sync is enabled)
            // Note: We'll need access to iCloudSyncService for this
            // For now, we'll rely on the NavUnitDetailsViewModel to handle CloudKit deletion
            // or we need to pass the iCloudSyncService to this ViewModel
